...
void NetworkConnectorEngine::aboutToFinish() {
  QLOG_STR("NetworkConnectorEngine cleanup for thread " + QString::number((long)(QThread::currentThreadId()))  ) ; 
  iModel->lock() ; 
  iModel->threadTerminationCleanup(this) ; 
  iModel->unlock() ; 
  QLOG_STR("NetworkConnectorEngine cleanup out for thread " + QString::number((long)(QThread::currentThreadId()))  ) ; 
}
