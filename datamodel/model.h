/*    -*-C++-*- -*-coding: utf-8-unix;-*-
  Classified Ads is Copyright (c) Antti Jarvinen 2013-2022.
...
class Model : public MModelProtocolInterface {
 ...
 public:
  ...
  ContentEncryptionModel& contentEncryptionModel()
      const; /**< method for getting content-encryption-specific datamodel */
  virtual ProfileModel& profileModel()
      const; /**< method for getting profile-specific datamodel */
  virtual BinaryFileModel& binaryFileModel()
      const; /**< method for getting binary-blob datamodel */
  virtual ClassifiedAdsModel& classifiedAdsModel()
      const; /**< method for getting the ads datamodel */
  virtual PrivMessageModel& privateMessageModel()
      const; /**< method for getting the priv msg datamodel */
  virtual ProfileCommentModel& profileCommentModel()
      const; /**< method for getting the comment datamodel */
  virtual SearchModel* searchModel()
      const; /**< method for getting the full text search datamodel */
  virtual CaDbRecordModel* caDbRecordModel()
      const; /**< method for getting distributed database model part */
  virtual TrustTreeModel* trustTreeModel()
      const; /**< method for getting the trust tree datamodel */
  virtual TclModel& tclModel()
      const; /**< method for getting the storage of TCL programs */
  /**
   ...
   */
  QList<NetworkRequestExecutor::NetworkRequestQueueItem>& getNetRequests()
      const;
  /**
   ...
   * @return none
   */
  virtual void addNetworkRequest(
   ...
   * @param aSecondaryAddr is possible 2nd publish addr, used for CA and
   * comment. for private messages this is the destination node, if known.
   */
  void addItemToBePublished(ProtocolItemType aType, const Hash& aHash,
   ...
   *         move to next.
   */
  PublishItem addNodeToPublishedItem(const Hash& aContentHash,
 ...
 private:
  ...
  CaDbRecordModel*
      iCaDbRecordModel;            /**< distributed general-purpose database */
...
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
  OSSL_PROVIDER* iLegacyCryptoProvider;
  OSSL_PROVIDER* iDefaultCryptoProvider;
#endif
};
#endif
