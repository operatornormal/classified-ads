...
ContentEncryptionModel::ContentEncryptionModel(MController *aController,
        const MModelProtocolInterface &aModel) : iController(aController),
    iModel(aModel) {
...
Hash ContentEncryptionModel::generateKeyPair() {
    ...
    if((pri == NULL || pub == NULL ) ||
            (!PEM_write_bio_PrivateKey(pri, pkey,  EVP_bf_cbc(), NULL, 0, 0, (void*)qPrintable(iController->contentKeyPasswd())))) {
...
int ContentEncryptionModel::changeKeyPassword(const Hash& aFingerPrint,
    ...
    if ( PrivateKey(aFingerPrint, pemBytes) ) {
        ...
        if ( keyToModify ) {
            ...
            BIO *pri = BIO_new(BIO_s_mem());
            if (!PEM_write_bio_PrivateKey(pri, keyToModify,  EVP_bf_cbc(), NULL, 0, 0, (void*)qPrintable(iController->contentKeyPasswd()))) {
                QString errmsg(ERR_reason_error_string(ERR_get_error())) ;
                emit error(MController::ContentEncryptionError, errmsg) ;
            } else {
                size_t pri_len = BIO_pending(pri);
...
bool  ContentEncryptionModel::verify(const QByteArray& aPemBytesOfSigningKey,
    ...
    return doVerify(aPemBytesOfSigningKey,
                    aDataToVerify,
                    aSignatureToVerify,
                    aOptionalMetadata,
                    emitErrorMessage ) ;
                ...
                if ( ek && ekl) {
                    ...
                    if ( alloc_failure == false ) {
                        if (!EVP_SealInit(&ctx, EVP_aes_256_cbc(), ek, ekl, iv, pubk, nr_of_valid_pubkeys)) {
                            ...
                            resultJSon.insert(KJSonEncryptedMessageIvElement,
                                              QString(QByteArray((const char*)iv,
                                                                 EVP_CIPHER_iv_length(EVP_aes_256_cbc())).toBase64()));
                            QVariantList listOfEncryptionKeys;
                            for ( k = 0 ; k < nr_of_valid_pubkeys; k++ ) {
                                if ( ekl[k] && ek[k] ) {
                                    listOfEncryptionKeys.append(QString(QByteArray((const char*)ek[k],
                                                                        ekl[k]).toBase64())) ;
                                    LOG_STR2("Ek: %s", qPrintable(QString(QByteArray((const char*)ek[k],
                                                                          ekl[k]).toBase64())));
...
#define FOUR_KBYTES 4096
                            ...
                            while ( pos < aPlainText.size() ) {
                                ...
                                if (!EVP_SealUpdate(&ctx,
                                    ...
                                    if ( len_out > 0 ) {
                                        aResultingCipherText.append((const char *)buffer_out,len_out) ;
                                    }
                                }
                            }
    ...
    if ( aCipherText.length() > 256 ) {
        ...
        if ( metaDataLen < (quint32)(aCipherText.length()) ) {
            ...
            if (ok &&
                ...
                const QByteArray iv ( QByteArray::fromBase64 (result[KJSonEncryptedMessageIvElement].toByteArray())) ;
                QVariantList encryptionKeyList( result[KJSonEncryptedMessageEkElement].toList() ) ;
    ...
    if ( hashOfPublicKey(aPublicKey) == aFingerPrintOfKey ) {
        if ( PublicKey(aFingerPrintOfKey, dummy) ) {
            ...
            if ( aDisplayName && aDisplayName->length() > 0 ) {
                ...
                ret = query.prepare ("update profile set time_last_reference=:time_last_reference, pubkey = :pubkey where hash1 = :hash1 and hash2 = :hash2 and hash3 = :hash3 and hash4 = :hash4 and hash5 = :hash5" ) ;
            }
            if ( ret ) {
            ...
            if ( aDisplayName && aDisplayName->length() > 0 ) {
                ...
                ret = query.prepare ("insert into profile (hash1,hash2,hash3,hash4,hash5,pubkey,time_last_reference) values (:hash1,:hash2,:hash3,:hash4,:hash5,:pubkey,:time_last_reference)" ) ;
            }
            if ( ret ) {
                query.bindValue(":hash1", aFingerPrintOfKey.iHash160bits[0]);
                query.bindValue(":hash2", aFingerPrintOfKey.iHash160bits[1]);
            ...
            ret = query.exec() ;
            QLOG_STR("### inserted profile encryption public key " + aFingerPrintOfKey.toString()) ;
            if ( !ret ) {
                QLOG_STR(query.lastError().text() + " "+ __FILE__ + QString::number(__LINE__)) ;
                emit error(MController::DbTransactionError, query.lastError().text()) ;
            } else {
...
bool ContentEncryptionModel::PrivateKey (const Hash& aFingerPrintOfKeyToFind,
    ...
    if ( !ret ) {
        LOG_STR("Error while doing select prikey from privatekeys where hash1 = :hash1 and hash2 = :hash2 and hash3 = :hash3 and hash4 = :hash4 and hash5 = :hash5") ;
        emit error(MController::DbTransactionError, query.lastError().text()) ;
    }
...
EVP_PKEY *ContentEncryptionModel::PrivateKeyFromPem(const QByteArray& aPemBytes,
    ...
    retval = PEM_read_bio_PrivateKey(pri, NULL,
                                     NULL, (void*)qPrintable(iController->contentKeyPasswd()));
    BIO_free(pri) ;
    if ( !retval ) {
