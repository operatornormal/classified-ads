...
const int KScaledImageMaxDimension(1024);

TextEdit::TextEdit(QWidget *aParent, MController *aController,
...
TextEdit::~TextEdit() { delete iDialogMenuBar; }
...
void TextEdit::addAction(QAction *aAction) { tb->addAction(aAction); }
...
void TextEdit::closeEvent(QCloseEvent *e) { e->accept(); }
...
void TextEdit::setupEditActions() {
  ...
  connect(actionInsertLink, SIGNAL(triggered()), this,
          SLOT(insertLinkSelected()));
  menu->addAction(actionInsertLink);
  actionInsertImage = new QAction(tr("Embed image"), this);
  connect(actionInsertImage, SIGNAL(triggered()), this,
          SLOT(insertImageSelected()));
...
bool TextEdit::maybeSave() {
  ...
  if (fileName.startsWith(QLatin1String(":/"))) return true;
...
void TextEdit::insertLinkSelected() {
  ...
  InsertLinkDialog *link_dialog = new InsertLinkDialog(this, iController);
  connect(link_dialog,
          SIGNAL(error(MController::CAErrorSituation, const QString &)),
          iController,
          SLOT(handleError(MController::CAErrorSituation, const QString &)),
          Qt::QueuedConnection);
  connect(this, SIGNAL(accepted()), link_dialog, SLOT(deleteLater()),
          Qt::QueuedConnection);
  connect(this, SIGNAL(rejected()), link_dialog, SLOT(deleteLater()),
          Qt::QueuedConnection);

  // don't connect as queued, it transfers pointers
  assert(connect(link_dialog,
                 SIGNAL(linkReady(const QString &, const QString &)), this,
                 SLOT(linkReady(const QString &, const QString &))));
...
void TextEdit::insertImageSelected() {
  ...
  if (fileName.length() > 0) {
    ...
    if (!pic.load(fileName)) {
      QMessageBox::about(this, tr("Error"), tr("Can't load image"));
      return;
    } else {
      imageIsOk = true;
    }
  ...
  if (imageIsOk) {
    ...
    if (strlen(fileNameCStyleHeap) < 1024) {
      ...
      /*MAGIC_MIME tells magic to return a mime of the file, but you can specify
       * different things*/
    ...
    if (mimeType.length() > 0) {
      ...
      if (f.open(QIODevice::ReadOnly)) {
        if (f.size() > (KMaxProtocolItemSize / 2)) {
          LOG_STR2("File size too big = %d", (int)(f.size()));
          // file too big, scale:
          f.close();
          ...
          if (originalImage.width() > originalImage.height()) {
            // image width is bigger than height -> scale width
            LOG_STR2("Scaling from width = >%d<", (int)(originalImage.width()));
            scaledImage = originalImage.scaledToWidth(KScaledImageMaxDimension);
          } else {
            // image height is bigger than width -> scale height
            LOG_STR2("Scaling from height = >%d<",
                     (int)(originalImage.height()));
            scaledImage =
                originalImage.scaledToHeight(KScaledImageMaxDimension);
          }
          QBuffer imageBuffer(&content);
          imageBuffer.open(QIODevice::WriteOnly);
          scaledImage.save(&imageBuffer, "JPG");
          imageBuffer.close();
          mimeType = "image/jpeg";
        } else {
          // read with no scale
          content.append(f.read(f.size()));
        }
        QString snippet = "<img alt=\"" + QFileInfo(f).fileName() +
                          "\" src=\"data:" + mimeType + ";base64," +
                          content.toBase64() + "\"/>";
        content.clear();
        if (((quint32)(snippet.length()) +
             (quint32)(textEdit->toHtml().length()) + 10240) >
...
void TextEdit::linkReady(const QString &aLinkAddress,
  ...
  textEdit->insertHtml("<a href='" + aLinkAddress + "'>" + aLinkLabel +
                       "</a> ");
}
